...
import { Play, Pause, Square, Wind, Star, X, Heart, ScanFace, Footprints, Check } from 'lucide-react';
...
const MeditationHall: React.FC<MeditationHallProps> = ({ language }) => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center h-full overflow-y-auto p-6 bg-gradient-to-br from-stone-50 to-orange-50 rounded-2xl shadow-inner text-center">
      
      <div className="mb-6 shrink-0">
        <h2 className={`text-3xl font-serif text-stone-800 mb-2 ${language === 'si' ? 'font-sinhala' : ''}`}>
           {isActive ? (language === 'si' ? 'හුස්ම ගන්න...' : 'Breathe...') : (language === 'si' ? 'භාවනා ශාලාව' : 'Meditation Hall')}
        </h2>
        <p className={`text-stone-500 flex items-center justify-center gap-2 ${language === 'si' ? 'font-sinhala' : ''}`}>
            {getIcon(selectedMethod.icon, 16)}
      ...
      <div className="flex items-center space-x-6 mb-8 shrink-0">
        {!isActive ? (
             <button onClick={toggleTimer} className="w-16 h-16 bg-orange-600 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-orange-700 transition-all transform hover:scale-105">
      ...
      {/* Guidance Text Area */}
      <div className="max-w-md w-full min-h-[60px] mb-4 shrink-0">
          {loadingGuidance ? (
              <p className="text-stone-400 italic animate-pulse">Preparing guidance...</p>
          ) : (
            guidance && (
                <div className="bg-white/80 p-4 rounded-xl border border-orange-100 shadow-sm">
                    <p className={`text-stone-700 italic font-serif ${language === 'si' ? 'font-sinhala' : ''}`}>"{guidance}"</p>
                </div>
            )
          )}
      </div>
                      ...
                      {MEDITATION_TYPES.map((method) => (
                          <button
                            ...
                            onClick={() => {
                                ...
                                setGuidance(''); // Reset guidance when method changes
                            }}
                          ...
                          >
                              <div className={`flex items-center gap-2 mb-1 ${selectedMethod.id === method.id ? 'text-orange-600' : 'text-stone-600'}`}>
                                  {getIcon(method.icon, 18)}
                                  <span className={`font-bold text-sm ${language === 'si' ? 'font-sinhala' : ''}`}>
                                      {language === 'si' ? method.title.si : method.title.en}
                                  </span>
                              </div>
                              <p className={`text-xs text-stone-500 leading-tight ${language === 'si' ? 'font-sinhala' : ''}`}>
                                  {language === 'si' ? method.desc.si : method.desc.en}
                              </p>
                              {selectedMethod.id === method.id && (
