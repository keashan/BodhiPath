...
const Dashboard: React.FC<DashboardProps> = ({ preferences, onLogout, onUpdatePreferences }) => {
  ...
  useEffect(() => {
    const handleDailyWisdom = async () => {
        ...
        let today: DailyDrop | null = savedToday ? JSON.parse(savedToday) : null;

        const now = Date.now();
        const oneDay = 24 * 60 * 60 * 1000;

        if (!today || (now - today.timestamp > oneDay)) {
            if (today && !history.find(d => d.timestamp === today?.timestamp)) {
                const updatedHistory = [today, ...history].slice(0, 10);
            ...
            localStorage.setItem('bodhi_drop_today', JSON.stringify(newDrop));
        } else {
            setDailyDrop(today);
        }
    };
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex-1 flex overflow-hidden">
            <div className="flex-1 overflow-y-auto p-4 md:p-12 custom-scrollbar">
                ...
                {currentView === AppView.TERMS && <LegalView type="terms" language={preferences.language} onBack={() => setCurrentView(AppView.DASHBOARD)} />}
                {currentView === AppView.PRIVACY && <LegalView type="privacy" language={preferences.language} onBack={() => setCurrentView(AppView.DASHBOARD)} />}
            </div>
            ...
            <aside className="hidden xl:flex w-80 bg-[#faf9f8] border-l border-stone-100 flex-col p-8 shrink-0">
                <div className="w-full h-full rounded-[2.5rem] bg-white border border-stone-100 shadow-sm flex flex-col items-center justify-center p-8 text-center relative overflow-hidden">
                    ...
                    <Logo className="w-16 h-16 opacity-10 mb-6" />
                    <p className="text-[10px] text-stone-300 font-bold uppercase tracking-[0.3em] mb-4">Awaiting Reflection</p>
                    <p className="text-xs text-stone-400 italic leading-relaxed font-serif">"Better than a thousand hollow words, is one word that brings peace."</p>
                </div>
            </aside>
        </div>
      </main>
